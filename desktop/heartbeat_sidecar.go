...
func (e *HeartbeatEngine) readRunHistorySidecar(cfg heartbeatConfig) map[string][]HeartbeatRun {
	...
	if sidecar.SchemaVersion > heartbeatRunHistorySchemaVersion {
		log.Printf("[heartbeat] run-history sidecar schemaVersion %d is newer than this binary supports (%d); ignoring it", sidecar.SchemaVersion, heartbeatRunHistorySchemaVersion)
		return nil
	}
