...
include `deepseek-v4-flash-vision-exp`. Select that SKU to send composer and
`@` user images; Flash and Pro remain text-only, and tool screenshots are not
forwarded as image parts. The vision SKU uses the Flash rate card. The dedicated
