...
对于明确要求修改的普通任务，若宿主尚未观察到成功 mutation，或本任务内成功
`todo_write` 创建的列表在 mutation 后仍有未完成项，Standard 最多自动追加两轮。
第二轮必须建立在新的宿主可验证进展上；完全相同的读取、命令、结果或纯文本不能换取
额外轮次。历史 canonical Todo 继续显示，但不会阻塞新的普通任务。Standard 下的验证、
复核和签收缺口仍只作为完成提示，不升级成 Delivery 强度的自动闭环。两轮耗尽后，
Reasonix 会以“任务尚未完成”暂停，并保留当前证据供 `/continue-checks` 恢复。
