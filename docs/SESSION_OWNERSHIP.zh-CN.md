...
路径型写入对工作区加 shared 锁、对具体文件加 exclusive 锁，且只在该次 tool
期间持有，因此两个会话可以同时写不同文件（包括同一仓库）。`bash`/MCP 的写操作
只在该命令期间独占工作区；若配置的 tool hook 可能写入未声明路径，任何 tool
调用都会改用整区锁。文件身份映射到有界的锁分片；哈希碰撞最多让无关文件串行，不会
削弱保护。只读 bash 不拿写锁。冲突卡片会说明正在写的文件或整区。对话结束后
不继续占锁。需要长期隔离工作树时再用 worktree。
